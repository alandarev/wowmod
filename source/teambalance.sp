		...
		switch(GetTeamBalance())
		{
			...
			case TEAMS_UNBALANCED:
			{
				...
				balanceTeams = true;
						...
						//case TEAM_SCRAMBLE:		{
						//	PrintTranslatedToChatAll("scramble");
						//	scrambleTeams=true;
						//}
		}
	}
...
//  Balances the teams based on Custom Rating
public bool:BalanceTeams()
{
	...
	SortCustom1D(arrayTeams[TERRORIST_INDEX], teamCount[TERRORIST_INDEX], SortRating);
	SortCustom1D(arrayTeams[COUNTER_TERRORIST_INDEX], teamCount[COUNTER_TERRORIST_INDEX], SortRating);
	...
	new Float:lowKDR = GetRating(arrayTeams[GetTeamIndex(GetOtherTeam(whoWonLast))][bottomPlayer]);
	for(new i = 0; i < numPlayers; i++)
	{
		if(GetRating(arrayTeams[GetTeamIndex(whoWonLast)][i + clientToSwitch]) > lowKDR)
		{
			goodPlayers++;
			if(bottomPlayer > 0)
			{
				bottomPlayer--;
				lowKDR = GetRating(arrayTeams[GetTeamIndex(GetOtherTeam(whoWonLast))][bottomPlayer]);
			}
		}
	}
...
public SortRating(elem1, elem2, const array[], Handle:hndl)	{
	if(GetRating(elem1) > GetRating(elem2))
	{
		return -1;
	} else if(GetRating(elem1) == GetRating(elem2)) {
		return 0;
	} else {
		return 1;
	}
...
public Float:GetRating(client)	{
	return wow_getTempPlayerRating(client, whoWonLast);
}
