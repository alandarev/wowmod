...
{
  name = "Team Bets",
  author = "GrimReaper (modified by stinkyfax)",
  description = "Bet wowmod Silver on Team to Win",
  version = PLUGIN_VERSION,
  url = "http://wowmod.eu"
};

#define MAXIMUM_SILVER 300
#define WIN_RATIO 0.6
...
public OnPluginStart()
{
  ...
  {
    g_bEnabled = false;
    PrintToServer("[SilverBets] - Unable to start, cannot find necessary send prop offsets.");
    return;
  }
  ...
  CreateConVar("sm_teambets_version", PLUGIN_VERSION, "SilverBets Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);

  RegConsoleCmd("say", Command_Say);
  RegConsoleCmd("say_team", Command_Say);

  g_hSmBet = CreateConVar("sm_bet", "0", "Enable team betting? (0 off, 1 on, def. 1)");
  ...
  g_hSmBetAnnounce = CreateConVar("sm_bet_announce", "1", "Announce 1 vs 1 situations (0 off, 1 on, def. 0)");
  g_hSmBetAdvert = CreateConVar("sm_bet_advert", "1", "Advertise plugin instructions on client connect? (0 off, 1 on, def. 1)");
  g_hSmBetPlanted = CreateConVar("sm_bet_planted", "1", "Prevent betting if the bomb has been planted. (0 off, 1 on, def. 0)");
...
public Action:Timer_DelayedHooks(Handle:timer)
{
  if (g_bEnabled)
  {
    ...
    PrintToServer("[SilverBets] - Loaded");
  }
}
...
public Action:Command_Say(client, args)
{
  ...
  if (strcmp(szParts[0],"bet",false) == 0)
  {
    ...
    {
      PrintToChat(client, "\x04[SilverBets]\x01 %t", "No bets after bomb planted");
      return Plugin_Handled;
    }

    if (GetClientTeam(client) <= 1)
    {
      PrintToChat(client, "\x04[SilverBets]\x01 %t", "Must Be On A Team To Vote");
      return Plugin_Handled;
    }

    if (g_bPlayerBet[client])
    {
      PrintToChat(client, "\x04[SilverBets]\x01 %t", "Already Betted");
      return Plugin_Handled;
    }

    if (GetConVarInt(g_hSmBetDeadOnly) == 1 && IsAlive(client))
    {
      PrintToChat(client, "\x04[SilverBets]\x01 %t", "Must Be Dead To Vote");
      return Plugin_Handled;
    }

    if (strcmp(szParts[1],"ct",false) != 0 && strcmp(szParts[1],"t", false) != 0)
    {
      PrintToChat(client, "\x04[SilverBets]\x01 %t", "Invalid Team for Bet");
      return Plugin_Handled;
    }
    ...
    if (strcmp(szParts[1],"ct",false) == 0 || strcmp(szParts[1],"t", false) == 0)
    {
      ...
      if (iAmount < 1)
      {
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Invalid Bet Amount");
        return Plugin_Handled;
      }

      if (iAmount > iBank || iBank < 1)
      {
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Not Enough Silver");
        return Plugin_Handled;
      }
      ...
      {
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Players Are Dead");
        return Plugin_Continue;
      }
      ...
      {
        iWin = RoundToNearest((float(iOdds[1]) / float(iOdds[0])) * float(iAmount) * WIN_RATIO);
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Bet Made", iOdds[1], iOdds[0], iWin, g_iPlayerBetData[client][BET_AMOUNT]);
      }
      else
      {
        iWin = RoundToNearest((float(iOdds[0]) / float(iOdds[1])) * float(iAmount) * WIN_RATIO);
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Bet Made", iOdds[0], iOdds[1], iWin, g_iPlayerBetData[client][BET_AMOUNT]);
      }
...
public bool:OnClientConnect(client, String:rejectmsg[], maxlen)
{
  ...
  CreateTimer(25.0, Timer_Advertise, client);
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  {
    PrintToChatAll("\x04[SilverBets]\x01 %T", "One Vs One", LANG_SERVER);
    return;
  }
  ...
  if (GetConVarInt(g_hSmBetOneVsMany) > 1)
  {
    ...
    if ((iTeams[0] == 1 && iTeams[1] > GetConVarInt(g_hSmBetOneVsMany)) || (iTeams[1] == 1 && iTeams[0] > GetConVarInt(g_hSmBetOneVsMany)) && !g_bOneVsMany)
    {
      ...
      PrintToChatAll("\x04[SilverBets]\x01 %T", "One Vs Many Start", LANG_SERVER, pname, (iTeams[1] == 1 ? "Terrorist" : "Counter-Terrorist"));
    }
    else if (iTeams[0] == 1 && iTeams[1] == 0 && g_bOneVsMany && g_iOneVsManyTeam == 2)
    {
      GetClientName(iTPlayer, pname, 64);
      PrintToChatAll("\x04[SilverBets]\x01 %T", "One Vs Many Winner", LANG_SERVER, pname, g_iOneVsManyPot);
      AddSilver(iTPlayer, g_iOneVsManyPot);
    }
    else if (iTeams[1] == 1 && iTeams[0] == 0 && g_bOneVsMany && g_iOneVsManyTeam == 3)
    {
      GetClientName(iCTPlayer, pname, 64);
      PrintToChatAll("\x04[SilverBets]\x01 %T", "One Vs Many Winner", LANG_SERVER, pname, g_iOneVsManyPot);
      AddSilver(iCTPlayer, g_iOneVsManyPot);
    }
  }
}
...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  for (new i = 1; i <= iMaxClients; i++)
  {
    if (IsClientInGame(i) && g_bPlayerBet[i])
    {
      ...
      {
        AddSilver(i,g_iPlayerBetData[i][BET_AMOUNT] + g_iPlayerBetData[i][BET_WIN]);
        PrintToChat(i, "\x04[SilverBets]\x01 %t", "Bet Won", g_iPlayerBetData[i][BET_WIN], g_iPlayerBetData[i][BET_AMOUNT]);
      }
      else
      {
        PrintToChat(i, "\x04[SilverBets]\x01 %t", "Bet Lost", g_iPlayerBetData[i][BET_AMOUNT]);
      }
    }
  ...
  if (GetConVarInt(g_hSmBetAdvert) == 1)
    {
      if (IsClientInGame(client))
        PrintToChat(client, "\x04[SilverBets]\x01 %t", "Advertise Bets");
...
public AddSilver(client, amount)
{
  new money = wow_getSilver(client);
  money += amount;
  wow_setSilver(client, money);
}
public TakeSilver(client, amount) {
  new money = wow_getSilver(client);
  money -= amount;
  wow_setSilver(client, money);
}

public GetSilver(client)
{
  new money = wow_getSilver(client);
  if(money < 0)
    money = 0;
  if(money > MAXIMUM_SILVER)
    money = MAXIMUM_SILVER;
  return money;
}
